document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!menuToggle || !mobileMenu || !overlay) {
    console.warn("Mobile nav: missing element(s)", {
      menuToggle,
      mobileMenu,
      overlay,
    });
...
// Event Registration Modal with Formspree via fetch
document.querySelectorAll(".register-event-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    formModal.innerHTML = `
      <div class="bg-white rounded-lg p-8 max-w-md w-full mx-4">
        <div class="text-center">
          <h3 class="text-xl font-bold text-gray-900 mb-4">Register for ${eventName}</h3>
          <form id="event-registration-form" class="space-y-4 text-left">
            <input type="hidden" name="eventName" value="${eventName}">
            <input type="hidden" name="eventDate" value="${eventDate}">
            <input type="hidden" name="eventPrice" value="${eventPrice}">
            <div>
              <label class="block text-gray-700 mb-1">Full Name</label>
              <input type="text" name="name" required class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div>
              <label class="block text-gray-700 mb-1">Email</label>
              <input type="email" name="email" required class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div>
              <label class="block text-gray-700 mb-1">Phone (optional)</label>
              <input type="tel" name="phone" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div class="flex justify-end space-x-2 mt-4">
              <button type="button" class="close-btn bg-gray-300 text-gray-800 px-4 py-2 rounded hover:bg-gray-400 transition">Cancel</button>
              <button type="submit" class="bg-primary text-white px-4 py-2 rounded hover:bg-primary/90 transition">Register</button>
            </div>
          </form>
        </div>
      </div>
    ...
    // Close button
    formModal.querySelector(".close-btn").onclick = () => formModal.remove();

    // Handle form submission
    formModal
      .querySelector("#event-registration-form")
      .addEventListener("submit", (event) => {
        event.preventDefault(); // stop normal submission

        const formData = new FormData(event.target);

        fetch("https://formspree.io/f/mnndwgrz", {
          method: "POST",
          headers: {
          ...
          .then((response) => {
            ...
            <div class="bg-white rounded-lg p-8 max-w-md w-full mx-4">
              <div class="text-center">
                ...
                <p class="text-gray-600 mb-6">Thank you ${formData.get(
                  "name"
                )} for registering for ${eventName}. We'll send a confirmation email to ${formData.get(
                "email"
              )}.</p>
...
document.querySelectorAll(".enroll-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    const programName = card.querySelector("h2").textContent;
